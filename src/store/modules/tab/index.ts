...
import { useRouterPush } from '@/composables'; // 组合（路由推送）
import { getTabRoutes } from '@/utils'; // 工具（获取选项卡路由）
import type { GlobalTabRoute } from '@/interface'; // 接口（全局选项卡路由）
// import { useThemeStore } from '../theme'; // 主题（主题状态储存）
import { getTabRouteByVueRoute, isInTabRoutes, getIndexInTabRoutes } from './helpers'; // 帮助函数
...
export const useTabStore = defineStore('tab-store', {
  ...
  actions: {
    ...
    iniTabStore(currentRoute: RouteLocationNormalizedLoaded) {
      // const theme = useThemeStore(); //设置主题

      const isHome = currentRoute.path === this.homeTab.path;
      // const tabs: GlobalTabRoute[] = theme.tab.isCache ? getTabRoutes() :[]; // //主题是否有缓存
      const tabs: GlobalTabRoute[] = getTabRoutes(); // 默认开始缓存
